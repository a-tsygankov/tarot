...
export class DashboardPanelLite extends LitElement {
    static override styles = [sharedStyles, css`
        ...
        .value { font-family: var(--font-display); color: var(--gold); font-size:1.35em; }
    ...
    private renderGate() {
        return html`
            <div class="panel">
                ...
                <div class="subtle" style="margin-bottom:.75em;">Load users, sessions, locations, questions, and game details.</div>
    ...
    private renderDashboard() {
        ...
        return html`
            ...
            ${this.error ? html`<div class="error">${this.error}</div>` : nothing}
            <div class="stats">
                ${this.stat(totals?.users, 'Users')}
                ${this.stat(totals?.sessions, 'Sessions')}
                ${this.stat(totals?.readings, 'Readings')}
                ${this.stat(totals?.followUps, 'Follow-ups')}
    ...
    private renderTab(d: JsonMap) {
        ...
        if (this.tab === 'sessions') {
            ...
            return html`<div class="panel"><div class="section">Sessions</div><div class="table-wrap"><table><thead><tr><th>Session</th><th>User</th><th>Location</th><th>Activity</th><th>Questions</th></tr></thead><tbody>
                ${sessions.map((s: any) => html`<tr>
                    ...
                    <td>${s.gameCount} games · ${s.device ?? 'unknown'} · v${s.appVersion}</td>
        ...
        if (this.tab === 'locations') {
            ...
            return html`<div class="panel"><div class="section">Locations</div><div class="table-wrap"><table><thead><tr><th>Location</th><th>Users</th><th>Sessions</th><th>Games</th><th>Last Played</th></tr></thead><tbody>
        ...
        return html`<div class="panel"><div class="section">Recent Games</div><div class="table-wrap"><table><thead><tr><th>Time</th><th>Game</th><th>User</th><th>Session</th><th>Location</th><th>Question</th></tr></thead><tbody>
    ...
    private renderDetail() {
        ...
        if (this.detailKind === 'game') {
            ...
            return html`${this.crumb('Game', this.short(String(game.gameId ?? '')))}
                <div class="panel"><div class="section">Game Detail</div><div class="grid">
        ...
        if (this.detailKind === 'session') {
            ...
            return html`${this.crumb('Session', this.short(String(session.sessionId ?? '')))}
                ...
                <div class="panel"><div class="section">Games In Session</div><div class="table-wrap"><table><thead><tr><th>Time</th><th>Game</th><th>Question</th><th>Location</th></tr></thead><tbody>${this.getArr(d, 'games').map((g: any) => html`<tr><td>${this.time(g.createdAt)}</td><td><button class="linkish" @click=${() => this.openDetail('game', g.gameId)}>${g.gameId}</button></td><td>${g.question ?? '-'}</td><td>${[g.location?.city, g.location?.country].filter(Boolean).join(', ') || '-'}</td></tr>`)}</tbody></table></div></div>
    ...
    private stat(value: unknown, label: string) { return html`<div class="stat"><div class="value">${value ?? '-'}</div><div class="subtle">${label}</div></div>`; }
